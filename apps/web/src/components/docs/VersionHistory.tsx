    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-[calc(100%+6px)] z-20 max-h-[360px] w-[300px] overflow-y-auto rounded-[9px] border border-border bg-surface shadow-kp">
          {loading && <p className="px-3 py-3 type-body text-text-3">{t("loadingEllipsis")}</p>}
          {!loading && data?.versions.length === 0 && (
            <p className="px-3 py-3 type-body text-text-3">{t("versionHistory.noHistoryYet")}</p>
          )}
          {!loading &&
            data?.versions.map((v) => {
              const author = v.authorId ? usersById.get(v.authorId) : undefined;
              return (
                <div key={v.id} className="flex items-center gap-2 border-b border-border px-3 py-2 type-body last:border-b-0">
                  <div className="min-w-0 flex-1">
                    <p className="truncate type-label">{new Date(v.createdAt).toLocaleString(intlLocale)}</p>
