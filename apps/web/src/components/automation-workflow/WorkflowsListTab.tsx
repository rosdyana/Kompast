...
export function WorkflowsListTab({ projectId, teamId, projectKey }: { projectId: string; teamId: string; projectKey: string }) {
  ...
  if (workflows === null) return <p className="p-6 type-body text-text-3">Loading…</p>;
  ...
  return (
    <div className="flex flex-col gap-4 p-6">
      ...
      <div className="flex flex-col gap-2">
        {workflows.length === 0 && <p className="type-body text-text-3">No workflows yet — create one above.</p>}
        {workflows.map((wf) => (
          <Link
            key={wf.id}
          ...
          >
            ...
            <Badge tone={wf.enabled ? "green" : "neutral"}>{wf.enabled ? "Enabled" : "Disabled"}</Badge>
            {wf.dryRun && <Badge tone="amber">Dry run</Badge>}
          </Link>
        ))}
      </div>
    </div>
